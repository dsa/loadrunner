(function loadrunner(context) {
    ...
    for (var i=0, file; file = files[i]; i++) {
      if (isModule(file)) {
        ...
        if (indexOf(using.loaded, file) > -1) {
          ...
          using.load(resolvePath(path), moduleFileLoadedCallback(file));
        }
      } else {
        if (indexOf(using.loaded, file) > -1) {
          fileLoaded();
        } else {
          using.load(resolvePath(file), fileLoaded);
  ...
  using.loaded = [];
  if (scriptTag) {
    using.path = scriptTag.getAttribute('data-path') ||
                 scriptTag.src.split(/loadrunner\.js/)[0] || '';
  }
  ...
  if (scriptTag && (main = scriptTag.getAttribute('data-main'))) {
